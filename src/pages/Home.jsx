    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none z-[5]">
      {[...Array(18)].map((_, i) => (
        <div key={i} className="absolute rounded-full" style={{
          ...
          background: `radial-gradient(circle, rgba(239,93,40,${Math.random() * 0.5 + 0.2}) 0%, transparent 70%)`,
    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-black">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence mode="sync">
          <motion.img key={currentSlide} src={heroImages[currentSlide]}
            alt={hero.backgroundImages?.[currentSlide]?.alt || 'Nu Aquarius Fitness'}
        ...
        <div className="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-[#EF5D28]/10 to-transparent z-[2]" />
      </motion.div>
      <NeonParticles />
      ...
      <div className="absolute right-5 sm:right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrentSlide(i)}
            className={`w-[3px] transition-all duration-700 ${i === currentSlide ? 'h-10 bg-[#EF5D28]' : 'h-4 bg-white/20 hover:bg-white/40'}`}
            aria-label={`Slide ${i + 1}`} />
        ))}
      </div>
      <div className="absolute top-[15%] left-0 w-[3px] h-32 sm:h-48 bg-gradient-to-b from-transparent via-[#EF5D28] to-transparent z-20"
        style={{ boxShadow: '0 0 20px rgba(239,93,40,0.4)' }} />
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36"
        ...
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }}
          ...
          className="w-16 h-[3px] bg-gradient-to-r from-[#EF5D28] to-[#EF5D28]/50 mb-6 origin-left"
          style={{ boxShadow: '0 0 15px rgba(239,93,40,0.5)' }} />
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.4 }}
          className="text-[#EF5D28] text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-8"
          style={{ fontFamily: 'var(--font-sans)' }}>
          {hero.badge || "Harare's Premier Fitness Destination"}
        </motion.p>
        <div className="overflow-hidden">
          {(hero.titleLines || ['PUSH', 'BEYOND', 'LIMITS.']).map((line, i) => (
            <motion.div key={line} initial={{ y: '110%' }} animate={{ y: 0 }}
              transition={{ duration: 1, delay: 0.5 + i * 0.15, ease: [0.16, 1, 0.3, 1] }}>
              <h1 className={`font-heading leading-[0.92] tracking-tight ${i === 1 ? 'text-[#EF5D28]' : 'text-white'}`}
                style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)', fontWeight: i === 1 ? 800 : 300,
                  textShadow: i === 1 ? '0 0 40px rgba(239,93,40,0.3)' : 'none' }}>
                {line}
              </h1>
            </motion.div>
          ))}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 1.2 }}
          ...
          <div className="w-8 h-[2px] bg-[#EF5D28]/60" />
          <p className="text-white/30 text-xs sm:text-sm uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.trustLine || '4.5 Stars \u00b7 31+ Reviews \u00b7 Fifth Avenue, Harare'}
          </p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 1.4 }} className="flex flex-wrap gap-4 mt-10">
          <Link to="/contact"
            className="group relative inline-flex items-center gap-3 bg-[#EF5D28] text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:shadow-xl hover:shadow-[#EF5D28]/30"
            style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary || 'Start Your Journey'}
            <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
          </Link>
          <Link to="/services"
            className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:border-[#EF5D28]/50 hover:text-[#EF5D28]"
            style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }}
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2, ease: 'easeInOut' }}>
          <ArrowDown size={14} className="text-[#EF5D28]/50" />
        </motion.div>
      </motion.div>
    </section>
  );
...
function StatsStrip() {
  ...
  return (
    <section ref={ref} className="relative bg-black border-y border-[#EF5D28]/15 overflow-hidden">
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#EF5D28]/40 to-transparent" />
      <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#EF5D28]/40 to-transparent" />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-12">
          {displayStats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }}
              ...
              <div className="font-heading text-[#EF5D28] leading-none font-bold"
                style={{ fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', textShadow: '0 0 30px rgba(239,93,40,0.2)' }}>
              ...
              {i < displayStats.length - 1 && <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 h-16 w-[1px] bg-gradient-to-b from-transparent via-[#EF5D28]/15 to-transparent" />}
            </motion.div>
          ))}
        </div>
      </div>
...
function MarqueeTicker() {
  ...
  return (
    <section className="bg-[#EF5D28] py-4 sm:py-5 overflow-hidden">
    ...
    <section ref={ref} className="bg-neutral-950 py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="w-12 h-[3px] bg-[#EF5D28] mb-6" style={{ boxShadow: '0 0 10px rgba(239,93,40,0.4)' }} />
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-[#EF5D28]/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Programs</p>
              <h2 className="font-heading text-white leading-[0.92] font-bold" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                Train Your <span className="text-[#EF5D28]">Way</span>
              </h2>
            </div>
            <Link to="/services" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-[#EF5D28] transition-colors"
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }}
                ...
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`}
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-[#EF5D28]/10 font-heading text-6xl sm:text-7xl font-bold leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 bg-[#EF5D28] flex items-center justify-center">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 z-10">
                    ...
                    <div className="flex items-center gap-2 mt-3 text-[#EF5D28] group-hover:translate-x-1 transition-transform duration-300">
                      <span className="text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Explore</span>
                      <ArrowRight size={14} />
                    </div>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#EF5D28] to-[#EF5D28]/60 z-10" />
                </Link>
              </motion.div>
            );
          })}
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <div className="w-12 h-[3px] bg-[#EF5D28] mb-6" />
            <p className="text-[#EF5D28]/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Who We Are</p>
            <h2 className="font-heading text-white leading-[0.95] font-bold mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              More Than A <span className="text-[#EF5D28]">Gym</span>
            </h2>
            <p className="text-white/50 text-sm sm:text-base leading-relaxed mb-6 max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              {about?.description || "Located at the heart of Harare on Fifth Avenue, Nu Aquarius Fitness Centre is where passion meets performance. We've built a community of dedicated athletes, weekend warriors, and first-time gym-goers \u2014 all unified by the desire to be better than yesterday."}
            </p>
            <div className="space-y-4 mb-10">
              {points.map((point, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -20 }}
                  ...
                  <div className="w-6 h-6 bg-[#EF5D28] flex items-center justify-center shrink-0">
            ...
            <Link to="/about" className="group inline-flex items-center gap-3 text-[#EF5D28] text-sm uppercase tracking-[0.15em] font-semibold hover:text-white transition-colors"
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <div className="overflow-hidden">
              <img src="https://images.unsplash.com/photo-1571019614242-c5c5dee9f50b?w=800&q=80" alt="Nu Aquarius gym floor"
                className="w-full aspect-[4/5] object-cover object-center" loading="eager" />
            </div>
            <div className="absolute -top-3 -right-3 w-20 h-20 border-t-[3px] border-r-[3px] border-[#EF5D28]/40" />
            <div className="absolute -bottom-3 -left-3 w-20 h-20 border-b-[3px] border-l-[3px] border-[#EF5D28]/40" />
            <div className="absolute -bottom-6 -right-4 sm:-right-6 bg-[#EF5D28] text-white p-5 sm:p-6">
              <div className="text-center">
                <div className="font-heading text-3xl sm:text-4xl font-bold leading-none">{business.rating || 4.5}</div>
...
function SchedulePreview() {
  ...
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const classes = [
    { time: '06:00', name: 'Morning HIIT', trainer: 'Coach Tawanda', level: 'All Levels' },
    { time: '08:00', name: 'Strength & Conditioning', trainer: 'Coach Nyasha', level: 'Intermediate' },
    ...
    <section ref={ref} className="bg-black py-24 sm:py-32 lg:py-40">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="w-12 h-[3px] bg-[#EF5D28] mx-auto mb-6" />
          <p className="text-[#EF5D28]/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Today's Classes</p>
          <h2 className="font-heading text-white leading-[0.95] font-bold" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
            Class <span className="text-[#EF5D28]">Schedule</span>
          </h2>
        </motion.div>
        <div className="space-y-2">
          {classes.map((cls, i) => (
            <motion.div key={cls.name} initial={{ opacity: 0, x: -20 }}
              ...
              className="group flex items-center justify-between bg-neutral-950 border border-white/5 p-5 sm:p-6 hover:border-[#EF5D28]/20 hover:bg-[#EF5D28]/5 transition-all duration-500">
              <div className="flex items-center gap-4 sm:gap-8">
                <span className="text-[#EF5D28] font-heading text-xl sm:text-2xl font-bold w-16 sm:w-20">{cls.time}</span>
                <div>
              ...
              <div className="hidden sm:flex items-center gap-4">
                <span className="text-[#EF5D28]/40 text-xs uppercase tracking-wider border border-[#EF5D28]/20 px-3 py-1" style={{ fontFamily: 'var(--font-sans)' }}>{cls.level}</span>
                <ArrowRight size={16} className="text-white/10 group-hover:text-[#EF5D28] transition-colors" />
              </div>
            </motion.div>
          ))}
        </div>
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 0.6 }} className="text-center mt-10">
          <Link to="/services" className="group inline-flex items-center gap-3 border border-[#EF5D28]/30 text-[#EF5D28] px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-[#EF5D28]/10 transition-all duration-300"
    ...
    <section ref={ref} className="relative bg-neutral-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <Quotes size={48} weight="fill" className="text-[#EF5D28]/15 mx-auto mb-8" />
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex flex-col items-center gap-3">
                {t.avatar && <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover object-center border-2 border-[#EF5D28]/30" loading="eager" />}
                <div className="w-8 h-[2px] bg-[#EF5D28]" />
                ...
                <div className="flex items-center gap-0.5 mt-1">
                  {[...Array(t.rating || 5)].map((_, j) => <Star key={j} size={12} weight="fill" className="text-[#EF5D28]" />)}
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-[#EF5D28] hover:border-[#EF5D28]/30 transition-colors" aria-label="Previous"><CaretLeft size={16} /></button>
            <div className="flex gap-2">
              {testimonials.map((_, i) => (
                <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-[#EF5D28]' : 'w-3 bg-white/10'}`} aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-[#EF5D28] hover:border-[#EF5D28]/30 transition-colors" aria-label="Next"><CaretRight size={16} /></button>
          </div>
        </motion.div>
      </div>
    </section>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={homeCta?.backgroundImage || 'https://images.unsplash.com/photo-1576678927484-cc907957088c?w=1200&q=80'} alt="Nu Aquarius gym"
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }}
          ...
          <div className="w-16 h-[3px] bg-[#EF5D28] mx-auto mb-8" />
          <h2 className="font-heading text-white leading-[0.92] font-bold mb-8" style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>
            YOUR TRANSFORMATION<br /><span className="text-[#EF5D28]">STARTS HERE</span>
          </h2>
          <p className="text-white/50 text-sm sm:text-base lg:text-lg max-w-lg mx-auto mb-12 leading-relaxed" style={{ fontFamily: 'var(--font-sans)' }}>
            {homeCta?.subtitle || 'Join hundreds of members who have already transformed their lives at Nu Aquarius. Your first session is on us.'}
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact"
              className="group inline-flex items-center gap-3 bg-[#EF5D28] text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold transition-all duration-500 hover:shadow-xl hover:shadow-[#EF5D28]/25"
              style={{ fontFamily: 'var(--font-sans)' }}>
              {homeCta?.ctaPrimary || 'Claim Free Trial'} <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" />
            </Link>
            <a href={`https://wa.me/${business.whatsappNumber || '263772480803'}?text=${encodeURIComponent(homeCta?.whatsappText || "Hi Nu Aquarius! I'd like to learn about membership options.")}`}
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }}>
            <div className="w-12 h-[3px] bg-[#EF5D28] mb-6" />
            <p className="text-[#EF5D28]/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Find Us</p>
            <h2 className="font-heading text-white leading-[0.95] font-bold mb-8" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Visit <span className="text-[#EF5D28]">Nu Aquarius</span>
            </h2>
            <div className="space-y-5">
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 bg-[#EF5D28] flex items-center justify-center shrink-0"><MapPin size={18} weight="fill" className="text-white" /></div>
                <div><h4 className="text-white text-sm font-semibold mb-1" style={{ fontFamily: 'var(--font-sans)' }}>Address</h4>
                  <p className="text-white/40 text-sm" style={{ fontFamily: 'var(--font-sans)' }}>{business.address || '143 Cnr Five Avenue, Fifth St, Harare'}</p></div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 bg-[#EF5D28] flex items-center justify-center shrink-0"><Phone size={18} weight="fill" className="text-white" /></div>
                <div><h4 className="text-white text-sm font-semibold mb-1" style={{ fontFamily: 'var(--font-sans)' }}>Phone</h4>
                  <a href={`tel:${business.phone || '+263772480803'}`} className="text-white/40 text-sm hover:text-[#EF5D28] transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
                    {business.phone || '+263 77 248 0803'}</a></div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 bg-[#EF5D28] flex items-center justify-center shrink-0"><WhatsappLogo size={18} weight="fill" className="text-white" /></div>
                <div><h4 className="text-white text-sm font-semibold mb-1" style={{ fontFamily: 'var(--font-sans)' }}>WhatsApp</h4>
                  <a href={`https://wa.me/${business.whatsappNumber || '263772480803'}`} target="_blank" rel="noopener noreferrer"
                    className="text-white/40 text-sm hover:text-[#EF5D28] transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>Message us on WhatsApp</a></div>
              </div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.8, delay: 0.15 }} className="relative aspect-[4/3] overflow-hidden">
            <img src="https://images.unsplash.com/photo-1540497077202-7c8a3999166f?w=800&q=80" alt="Nu Aquarius location"
            ...
            <div className="absolute bottom-5 left-5 z-10 flex items-center gap-2 bg-black/80 backdrop-blur-sm px-4 py-2">
              <MapPin size={14} className="text-[#EF5D28]" />
              <span className="text-white text-xs uppercase tracking-wider" style={{ fontFamily: 'var(--font-sans)' }}>Fifth Avenue, Harare</span>
            </div>
          </motion.div>
        </div>
      </div>
